  ...
  if (!token) {
    ...
    fetch(GRAPHQL_ENDPOINT, {
      ...
      .then(data => {
        ...
        nameContainer.textContent = `${user.firstName} ${user.lastName}`
      })
      .catch(error => {
        console.error(error)
      })
  ...
  if (!token) {
    ...
    fetch(GRAPHQL_ENDPOINT, {
      ...
      .then(data => {
        ...
        xpContainer.textContent = `Total amount of xp: ${amountInKb} kb`
...
async function fetchAudit(token, GRAPHQL_ENDPOINT) {
  ...
  try {
    ...
    const givenAmountInKb = givenTotalAmount / 1000;
    const receivedAmountInKb = receivedTotalAmount / 1000;

    donutChartMaker(givenAmountInKb, receivedAmountInKb);

    auditDone.textContent = `Total amount of audit xp done: ${givenAmountInKb} kb`;
    auditReceived.textContent = `Total amount of audit xp received: ${receivedAmountInKb} kb`;
  } catch (error) {
    console.error(error);
  }
}
...
async function fetchAttempts(token, GRAPHQL_ENDPOINT) {
  const attemptsQuery = `
    query {
      progress(where: {
